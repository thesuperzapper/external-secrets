...
func (r *Reconciler) getProviderSecretData(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, ssInfoMap, cssInfoMap map[string]*StoreInfo) (map[string][]byte, error) {
	...
	for i, remoteRef := range externalSecret.Spec.DataFrom {
		...
		if remoteRef.Find != nil {
			...
			usedKey = remoteRef.Extract.Key
			secretMap, err = r.handleExtractSecrets(ctx, externalSecret, remoteRef, mgr)
			if err != nil {
				err = fmt.Errorf("error processing spec.dataFrom[%d].extract (key: %s), err: %w", i, usedKey, err)
			}
		...
		if errors.Is(err, esv1beta1.NoSecretErr) && externalSecret.Spec.Target.DeletionPolicy != esv1beta1.DeletionPolicyRetain {
			if usedKey != "" {
				r.recorder.Eventf(externalSecret, v1.EventTypeNormal, esv1beta1.ReasonMissingProviderSecret, eventMissingProviderSecretFromKey, i, usedKey)
			} else {
				r.recorder.Eventf(externalSecret, v1.EventTypeNormal, esv1beta1.ReasonMissingProviderSecret, eventMissingProviderSecretFrom, i)
			}
			continue
		}
		if err != nil {
	...
	for i, secretRef := range externalSecret.Spec.Data {
		...
		if errors.Is(err, esv1beta1.NoSecretErr) && externalSecret.Spec.Target.DeletionPolicy != esv1beta1.DeletionPolicyRetain {
			r.recorder.Eventf(externalSecret, v1.EventTypeNormal, esv1beta1.ReasonMissingProviderSecret, eventMissingProviderSecretData, i, secretRef.RemoteRef.Key)
			continue
		}
		if err != nil {
...
func (r *Reconciler) handleSecretData(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, secretRef esv1beta1.ExternalSecretData, providerData map[string][]byte, cmgr *secretstore.Manager) error {
	_, client, err := cmgr.Get(ctx, externalSecret.Spec.SecretStoreRef, externalSecret.Namespace, toStoreGenSourceRef(secretRef.SourceRef))
	if err != nil {
		return err
	}
...
func (r *Reconciler) handleExtractSecrets(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, remoteRef esv1beta1.ExternalSecretDataFromRemoteRef, cmgr *secretstore.Manager) (map[string][]byte, error) {
	_, client, err := cmgr.Get(ctx, externalSecret.Spec.SecretStoreRef, externalSecret.Namespace, remoteRef.SourceRef)
	if err != nil {
		return nil, err
	}
...
func getInClusterKubernetesNamespace(provider *esv1beta1.SecretStoreProvider) string {
	...
	case "", "kubernetes.default", "kubernetes.default.svc", "kubernetes.default.svc.cluster.local":
