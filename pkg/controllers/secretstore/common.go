...
func reconcile(ctx context.Context, req ctrl.Request, ss esapi.GenericStore, cl client.Client, log logr.Logger,
	controllerClass string, gaugeVecGetter metrics.GaugeVevGetter, requeueInterval time.Duration) (result ctrl.Result, err error) {
	...
	err = validateStore(ctx, req.Namespace, controllerClass, ss, cl, gaugeVecGetter)
	if err != nil {
